...
# Custom
    def create_purchase_order(self, vendor, item, qty, rate, date):
        po = Form(request.env['purchase.order'])
        po.partner_id = request.env['res.partner'].search([['name', '=', vendor]])
        with po.order_line.new() as po_line:
            po_line.product_id = request.env['product.product'].search([['name', '=', item]])
            po_line.product_qty = qty
            po_line.price_unit = rate
        po.date_order = date
        po = po.save()
        po = Form(po)
        po = po.save()
    ...
    @http.route('/create_po', type='json', auth='user')
    def create_po(self, **rec):
        if request.jsonrequest:
            if rec['vendor']:
                self.create_purchase_order(vendor=rec['vendor'], item=rec['item'], qty=rec['qty'], rate=rec['rate'], date=rec['date'])
